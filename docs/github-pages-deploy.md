...
This document describes how `.github/workflows/deploy-pages.yml` publishes the static site under `website/` to GitHub Pages (added in PR #4; `enablement` removed in PR #5).
...
| 2 | `actions/configure-pages@v5` | Configure Pages for this repo (no `enablement` input) |
...
GitHub Pages must be enabled for the repository (**Settings → Pages**), typically with **GitHub Actions** as the source. The workflow does **not** pass `enablement: true` to `configure-pages` — that flag caused a permission error on this fork (PR #5) and was removed.
...
4. **Do not re-add `enablement: true`** — It failed with a permission error here. Enable Pages in repo settings (or org policy) instead of asking the workflow to auto-enable.
