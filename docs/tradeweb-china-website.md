...
`website/index.html` is a **self-contained marketing/info page** that summarizes Tradeweb’s China access channels for the onshore bond and derivatives markets. It is **not** part of the Changesets GitHub Action runtime (`src/`, `action.yml`, workflows).

Use it when you need a local, dependency-free preview of the channel timeline and deep links to Tradeweb’s emerging-markets pages.
...
There is no build step. Opening or serving the HTML file is the full delivery path.
...
2. **No deploy pipeline in-repo** — As of this writing there is no GitHub Pages workflow or `package.json` script that publishes `website/`. Serving is manual unless you add hosting elsewhere.
