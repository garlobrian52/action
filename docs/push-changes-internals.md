...
| **Attribution** | `github-actions[bot]` only if `setupGitUser` input is `true` (invokes `setupUser`, `src/git.ts` lines 58–76); otherwise uses any preconfigured runner git identity, and the commit fails if no git identity is configured | Attributed to the `GITHUB_TOKEN` owner (the user or app that owns the token) |
